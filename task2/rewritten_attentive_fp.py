...
class AttentiveFP(torch.nn.Module):
    ...
    def __init__(
        ...
        # self.lin1 = Linear(in_channels, hidden_channels)
        self.gcnconv=CurConv(hidden_channels,hidden_channels)
    ...
    def forward(self, x: Tensor, edge_index: Tensor, edge_attr: Tensor,
                batch: Tensor,curvature:Tensor) -> Tensor:
        """"""
        # Atom Embedding:
        
        x = F.leaky_relu_(self.lin1(x))
        
        x=F.elu_(self.gcnconv(x,edge_index,curvature))
        x=F.dropout(x,p=self.dropout,training=self.training)
        
        # h=F.elu_(self.gcnconv(x,edge_index,curvature))
        ...
        for conv, gru in zip(self.atom_convs, self.atom_grus):
            h = F.elu_(conv(x, edge_index,curvature))
            h = F.dropout(h, p=self.dropout, training=self.training)
            x = gru(h, x).relu_()
