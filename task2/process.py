...
def Mult_graph(lig_file_name, pocket_file_name, id, score,port_lm):
    ...
    if(mol != None) and (protein != None):
        G_l,lig_atom_fea_list = Ligand_graph(lig_atom_fea, mol, h_num_lig,score)
        G_p,pocket_atom_fea_list = protein_graph(pocket_atom_fea,protein,h_num_pro,score)
        G_inter = Inter_graph(lig_coord, pocket_coord, lig_atom_fea_list, pocket_atom_fea_list, score)
        G_list = [G_l, G_p, G_inter, pro_seq, id, ligand_center]
...
def protein_graph(pocket_atom_fea, protein,h_num,score):
    ...
    G=nx.Graph()
    for node in range(len(pocket_atom_fea)):
        G.add_node(node)
    ...
    for i, edge in enumerate(edges):
        n1, n2 = edge
    # 获取曲率值
        curvature_value = orc.G[n1][n2]["ricciCurvature"]
        # 将曲率值添加到边特征的末尾
        edges_fea[i].append(curvature_value)
        
    pocket_atoms_fea_list=pocket_atom_fea.tolist()
    
    for i in range(len(pocket_atoms_fea_list)):
        pocket_atoms_fea_list[i].append(orc.G.nodes[i].get('ricciCurvature', None)) 
        if pocket_atoms_fea_list[i][-1] is None:
           pocket_atoms_fea_list[i][-1]=0   
    protein_name=os.path.splitext(os.path.basename(protein.title))[0]
    edge_filename=os.path.join('./curvature/edge',f'{protein_name}.txt')
    if os.path.exists(edge_filename):
        print("exists")
    else:
        with open(edge_filename,'w') as file:
            for item in edges_fea:
                file.write(str(item[-1])+'\n')
    node_filename=os.path.join('./curvature/node',f'{protein_name}.txt')
    if os.path.exists(node_filename):
        print("exists")
    else:
        with open(node_filename,'w') as file:
            for item in pocket_atoms_fea_list:
                file.write(str(item[-1])+'\n')
    
    edge_attr = torch.tensor(edges_fea, dtype=torch.float32)
    x = torch.tensor(pocket_atoms_fea_list, dtype=torch.float32)
    edge_index = edgelist_to_tensor(edges)
    G_pocket = Data(x=x, edge_index=edge_index, edge_attr=edge_attr, y=torch.tensor(score))

    return G_pocket,pocket_atoms_fea_list
...
def Ligand_graph(lig_atoms_fea,ligand,h_num,score):
    ...
    G=nx.Graph()
    for node in range(len(lig_atoms_fea)):
        G.add_node(node)
    G.add_edges_from(edges)
    orc = OllivierRicci(G, alpha=0.5, verbose="INFO")
    orc.compute_ricci_curvature()
    ...
    # 获取曲率值
        ...
        # 将曲率值添加到边特征的末尾
        edges_fea[i].append(curvature_value)
    lig_atoms_fea_list=lig_atoms_fea.tolist()
    for i in range(len(lig_atoms_fea_list)):
        lig_atoms_fea_list[i].append(orc.G.nodes[i].get('ricciCurvature', None))    
        if lig_atoms_fea_list[i][-1] is None:
            lig_atoms_fea_list[i][-1]=0 
    
   
    edge_filename=os.path.join('./curvature/edge',f'{ligand.title}.txt')
    if os.path.exists(edge_filename):
        print("exists")
    else:
        with open(edge_filename,'w') as file:
            for item in edges_fea:
                file.write(str(item[-1])+'\n')
    node_filename=os.path.join('./curvature/node',f'{ligand.title}.txt')
    if os.path.exists(node_filename):
        print("exists")
    else:
        with open(node_filename,'w') as file:
            for item in lig_atoms_fea_list:
                file.write(str(item[-1])+'\n')
    
    
    
    edge_attr = torch.tensor(edges_fea, dtype=torch.float32)
    x = torch.tensor(lig_atoms_fea_list, dtype=torch.float32)
    edge_index = edgelist_to_tensor(edges)
    G_lig = Data(x=x, edge_attr=edge_attr, edge_index=edge_index, y=torch.tensor(score))

    return G_lig,lig_atoms_fea_list
...
def process_raw_data(dataset_path, processed_file, protein_list):
    res = GetPDBDict(Path='/home/xyn/HaPPy-main/data/PDBbind_2016_plain_text_index/index/INDEX_general_PL_data.2016')
...
if __name__ == '__main__':
    ...
    raw_data_path = '/home/xyn/HaPPy-main/data/pdbfile/'
    general_list = GetPDBList(Path='/home/xyn/HaPPy-main/data/PDBbind_2016_plain_text_index/index/INDEX_general_PL_data.2016')
    refined_list = GetPDBList(Path='/home/xyn/HaPPy-main/data/PDBbind_2016_plain_text_index/index/INDEX_refined_data.2016')
    core_list = GetCorePDBList(Path='/home/xyn/HaPPy-main/data/CASF-2016/power_scoring/CoreSet.dat')

    general_set = set(general_list)
    refined_set = set(refined_list)
    core_set = set(core_list)

    print(len(general_set))
    print(len(refined_set))

    general_set = general_set - core_set
    refined_set = refined_set - core_set

    valid_protein_list = random.sample(list(refined_set), 1000)

    general_set = general_set - set(valid_protein_list)
  
    train_protein_list =list(general_set)

    test_protein_list = list(core_set)

    print('train_protein_list num: ', len(train_protein_list))
    print('valid_protein_list num: ', len(valid_protein_list))
    print('test_protein_list num: ', len(test_protein_list))

    process_raw_data(raw_data_path, './data/atom_conv/train.pkl', train_protein_list)
    process_raw_data(raw_data_path, './data/atom_conv/valid.pkl', valid_protein_list)
    process_raw_data(raw_data_path, './data/atom_conv/test.pkl', test_protein_list)

    # for item in test_protein_list:
    #     if item in train_protein_list:
    #         print(f"{item} 出现在train_protein_list中")
    #     elif item in valid_protein_list:
    #         print(f"{item} 出现在valid_protein_list中")
