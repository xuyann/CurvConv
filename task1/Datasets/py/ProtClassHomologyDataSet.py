...
class ProtClassHomologyDataSet:
    ...
    def compute_curvature(self,pAugment=False):
        ...
        for curIter in range(len(self.data_)):
            ...
            print(curIter)
            print("#########################")
            target_folder=f'/ProtHomology/curvature/test_superfamily'
            file_path=f'/ProtHomology/curvature/test_superfamily/curvature_{curIter}.txt'
            ...
            """print(len(curProtein.atomTypes_))
            
            
            print("######################")
            for neigh in curProtein.covBondListHB_:
                    
                print(neigh)
            print("-------------------------")
            for neigh in curProtein.aminoNeighsHB_:
                    
                print(neigh)
            print(len(curProtein.atomAminoIds_))"""
            G=nx.Graph()
            ...
            if not curProtein.covBondListHB_.any():
                ...
                file_name = f'curvature_{curIter}.txt'   
    ...
    def get_next_batch(self, flag,pBatchSize, pAugment = False):
        ...
        if self.iterator_ <= (len(self.data_)-pBatchSize):
            ...
            for curIter in range(pBatchSize):
                ...
                if flag==0:
                    source_folder = f'/ProtHomology/curvature/train' 
                elif flag==1:
                    source_folder = f'/ProtHomology/curvature/test_family'
                elif flag==2:
                    source_folder = f'/ProtHomology/curvature/test_fold'
                elif flag==3:
                    source_folder = f'/ProtHomology/curvature/test_superfamily'
                elif flag==4:
                    source_folder= f'/ProtHomology/curvature/validation'
                filename = f'curvature_{curProtIndex}.txt'
                file_path = os.path.join(source_folder, filename)
                with open(file_path, 'r') as file:
                    # 读取文件中的每一行，并将其转换为数字（假设每行包含一个数字）
                    for line in file:
