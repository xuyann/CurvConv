...
class TestLoop(with_metaclass(abc.ABCMeta)):
    ...
    def __init__(self, pConfig):
        ...
        #self.__compute_curvature__()

        #Init variables.
        self.sess_.run(tf.compat.v1.global_variables_initializer())
        self.sess_.run(tf.compat.v1.local_variables_initializer())
    ...
    def __create_tf_session__(self):
        ...
        #Create session
        gpuOptions = tf.compat.v1.GPUOptions(per_process_gpu_memory_fraction=self.gpuMem_, 
            visible_device_list=self.gpuId_)
        self.sess_ = tf.compat.v1.Session(config=tf.compat.v1.ConfigProto(gpu_options=gpuOptions))
    ...
    @abc.abstractmethod
    def __compute_curvature__(self, pNumEpoch):
