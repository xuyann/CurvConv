...
class ProteinPH(Protein):
    ...
    def __init__(self, pNumDims, pBatchSize, 
        ...
        if not pAminoInput:
            ...
            self.atomGraph2NeighsPH_.append(tf.placeholder(tf.int32, [None, 2]))
            self.atomGraph2NeighsStartIdsPH_.append(tf.placeholder(tf.int32, [None]))
            # self.edgeCurvature_.append(tf.placeholder(tf.float32,[None,10]))
        ...
        Protein.__init__(self, 
            ...
            pEdgeCurvature=self.edgeCurvature_)
    ...
    def update_dictionary(self, pCurDic, pProteinBatch):
        ...
        #     for edge_curate_tensor in pProteinBatch.edgeCurvature_:                
        #         numpy_array.append(sess.run(edge_curate_tensor))

        # pCurDic[self.edgeCurvature_]= numpy_array
        pCurDic[self.edgeLength]=pProteinBatch.edgeLength
        pCurDic[self.edgeCurvature_]= pProteinBatch.edgeCurvature_
        # print(pCurDic[self.edgeCurvature_])
