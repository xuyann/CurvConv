    ...
    def __init__(self, pProteinList, pAminoInput = False, pLoadText = False):
        ...
        if self.aminoInput_:
            ...
            for protIter, curProtein in enumerate(pProteinList):
                ...
                self.edgeCurvature_.append(curProtein.edgeCurvature_)
            ...
            self.edgeCurvature_=np.concatenate(self.edgeCurvature_,axis=0)
            ...
            self.edgeCurvature_ = np.concatenate(self.edgeCurvature_, axis=0)
            padding_rows = (-self.edgeCurvature_.shape[0]) % 4096

         
            if padding_rows > 0:
            
                padding = np.zeros((padding_rows, self.edgeCurvature_.shape[1]))
    
            
                self.edgeCurvature_ = np.vstack((self.edgeCurvature_, padding))
            
            self.edgeLength=self.edgeCurvature_.shape[0]
            self.atomBatchIds_ = np.concatenate(self.atomBatchIds_, axis=0)
            self.centers_ = np.concatenate(self.centers_, axis=0)
