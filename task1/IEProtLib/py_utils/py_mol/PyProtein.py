...
def func_k(curve):
    ...
    for i in range(len(k)):
        cur = (1 + np.exp(-k[i] * curve)) / 2

        if i == 0:
            Multi_curve = cur.reshape(-1, 1)
        else:
            Multi_curve = np.concatenate((Multi_curve, cur.reshape(-1, 1)), axis=1)
...
class PyProtein(PyMolecule):
    ...
    def load_hdf5(self, pFilePath, pLoadAtom = True, pLoadAmino = True, pLoadText = True):
        ...
        fileName,_=os.path.splitext(os.path.basename(pFilePath))
       
        source_folder = f'/Tasks/ProteinsDD/curvature_edge'
        filename = f'curvature_{fileName}.txt'
        file_path = os.path.join(source_folder, filename)
        protein_orc=np.array([])
        with open(file_path, 'r') as file:
            first_line=file.readline().strip()
            if first_line != 'not bond!':
                for line in file: 
                    number = float(line.strip())
                    if abs(number)<0.0000001 or np.isnan(number):
                        number=0.0000001
                    protein_orc = np.append(protein_orc, number)
        curva1 = func_k(protein_orc)  
